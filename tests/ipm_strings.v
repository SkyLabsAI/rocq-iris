(** * Imports copied from tests/proofmode.v *)
...
Notation byte_string := (@list Byte.byte) (only parsing).
Definition parse_string (s: list Byte.byte) : byte_string := id s.
Definition print_string (s: byte_string) : list Byte.byte := id s.
...
Delimit Scope byte_string_scope with bs.

Parameter PROP : bi.

(* Fails without [to_str] instance. *)
Goal True ⊢@{PROP} False.
Proof.
  Fail iIntros "H".
Abort.

Definition bs_to_string (s : byte_string) : String.string :=
  List.fold_right (fun b s => String (Ascii.ascii_of_byte b) s) String.EmptyString s.
Canonical byte_string_to_str := ToStr bs_to_string.

Goal True ⊢@{PROP} False.
Proof.
  iIntros "H".
  iDestruct "H" as "H".
  iAssert True%I with "[H]" as "H".
  { iApply "H". }
  iSpecialize "H".
Abort.

(** * Copy of tests/proofmode.v

    NOTE: There is a single [%string] annotation in the code below that is not
    easy to work around.
 *)
...
Proof.
  iIntros "? HQ H".
  iMatchHyp (fun H _ =>
    iApply ("H" with [spec_patterns.SIdent H []; spec_patterns.SIdent "HQ"%string []])).
Qed.
